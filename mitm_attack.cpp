...
int main(int argc, char **argv) {
    ...
    info.getInfo(interface);

    // Find interface index from interface name and store index in
    // struct sockaddr_ll device, which will be used as an argument of sendto().
    if ((info.device.sll_ifindex = if_nametoindex(interface)) == 0) {
    ...
    // printf("Index for interface %s is %i\n", interface, info.device.sll_ifindex);
    ...
    sendARPRequest(sd, info);

    // Use a table to save IP-MAC pairs
    std::map<std::vector<uint8_t>, std::vector<uint8_t>> ip_mac_pairs;

    // Start the thread
    std::thread send_thread(sendSpoofedARPReply, sd, std::ref(ip_mac_pairs), info);
    ...
    // Receive responses
    receiveHandler(sd, ip_mac_pairs, info);
